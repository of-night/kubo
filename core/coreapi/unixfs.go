...
// TEE Add builds a merkledag node from a reader, adds it to the blockstore,
// and returns the key representing that node.
func (api *UnixfsAPI) TEECRMAdd(ctx context.Context, theNewMultiReader *ipfsKeystoneTest.TheNewDirMultiProcessCrossTEEFileFlexibleReaderJustCall, files files.Node, opts ...options.UnixfsAddOption) (path.ImmutablePath, error) {
	ctx, span := tracing.Span(ctx, "CoreAPI.UnixfsAPI", "Add")
	defer span.End()
	...
	nd, err := fileAdder.AddAllAndPinTEECRM(ctx, files, theNewMultiReader)
	if err != nil {
		return path.ImmutablePath{}, err
	}
...
// TEE Add builds a merkledag node from a reader, adds it to the blockstore,
// and returns the key representing that node.
func (api *UnixfsAPI) TEEKeystoneAdd(ctx context.Context, theNewKeystoneReader *ipfsKeystoneTest.TheNewDirTEEFileReaderJustCallADD, files files.Node, opts ...options.UnixfsAddOption) (path.ImmutablePath, error) {
	ctx, span := tracing.Span(ctx, "CoreAPI.UnixfsAPI", "Add")
	defer span.End()
	...
	nd, err := fileAdder.AddAllAndPinTEEKeystone(ctx, files, theNewKeystoneReader)
	if err != nil {
		return path.ImmutablePath{}, err
	}
